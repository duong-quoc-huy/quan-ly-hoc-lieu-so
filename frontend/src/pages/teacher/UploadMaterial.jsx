...
export default function UploadMaterial() {
  ...
  const [processingStatus, setProcessingStatus] = useState(""); // Live status message
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (res.status === "completed") {
            ...
            clearInterval(interval);
            reject(new Error(res.error_message || "PPTX processing failed on Celery worker."));
          } else {
            setProcessingStatus(`Celery Worker: ${res.status.toUpperCase()} presentation...`);
          }
        } catch (err) {
          clearInterval(interval);
          reject(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Process any PPTX file asynchronously first to prevent Nginx 504 timeouts
      const pptxFile = files.find(f => f.name.toLowerCase().endsWith('.pptx'));
      if (pptxFile) {
        setProcessingStatus("Uploading presentation to Celery worker queue...");
        const pptxDoc = await materialService.uploadPPTXDocument(pptxFile);
        setProcessingStatus("Celery worker processing presentation slides...");
        await pollPptxProcessing(pptxDoc.id);
      }

      // 2. Update status for final package compilation
      setProcessingStatus("Presentation ready! Saving final material package...");

      // 3. Submit material package to main API
      ...
      if (existingSubmission) {
        result = await materialService.updateMaterial(existingSubmission.submission_id, formData);
      } else {
        result = await materialService.uploadMaterial(formData);
      }
      const responseData = result?.data ?? result;
      const targetId = 
        responseData?.submission_id || 
        responseData?.material_id || 
        responseData?.attachments?.[0]?.material_id || 
    ...
    <div className="min-h-full space-y-6 bg-slate-50/60 p-1 pb-12">
      ...
      <div className="mx-auto max-w-4xl">
        <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          {/* Dynamic Uploading & Celery Progress Banner */}
          {processingStatus && (
            <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-xs text-indigo-900 flex items-center gap-3 shadow-sm">
              <Loader2 className="w-5 h-5 text-indigo-600 animate-spin shrink-0" />
              <div>
                <p className="font-bold text-sm">Uploading & Processing</p>
                <p className="mt-0.5 text-indigo-800 font-medium">{processingStatus}</p>
              </div>
            </div>
          )}
